...
import ClickHandlerMusicXml from './click-handler-musicxml';
...
export default class MeiToolsAtomView {
  ...
  constructor() {
    ...
    // MusicXML: create Click handler
    this.xmlClickHandler = new ClickHandlerMusicXml();
    ...
    this.subscriptions = atom.workspace.getCenter().observeActivePaneItem(item => {
      ...
      if ((path.slice((path.lastIndexOf(".") - 1 >>> 0) + 2) !== "mei") && (path.slice((path.lastIndexOf(".") - 1 >>> 0) + 2) !== "xml") && (path.slice((path.lastIndexOf(".") - 1 >>> 0) + 7) !== "musicxml"))
       {
        this.verovioSVG.innerHTML = "<h2>This is not a MEI nor a MusicXML file. Notation will only be rendered for files ending in .mei, .xml or .musicxml that contain valid MEI or MusicXML markup.</h2>";
        this.hideByID(this.controlsForm.id);
        return;
      }
  ...
  updateAll(textEditor, options = {}) {
    ...
    this.xmlClickHandler.loadData(textEditor.getText(), this.vrvToolkit.getMEI());
  ...
  addNotationEventListeners(textEditor) {
    ...
    if (elements.length !== 0) {
      elements.attr('pointer-events', 'bounding-box')
  ...
  handleClickOnNotation(e, textEditor) {
    ...
    //// Retrieve cursor position in text buffer of selected element
    /// XML version
    // xmlClickHandler.handleClickOnNotation(e.currentTarget, textEditor);
    let buffer = textEditor.getBuffer();
    let range = this.xmlClickHandler.getRangeOfElement(element, buffer);
  ...
  updateHighlight(textEditor, updatePage = false) {
    //// Retrieve ID of graphical element at current text position
    let id = this.xmlClickHandler.getIdOfItemAtCursor(textEditor.getBuffer(), textEditor.getCursorBufferPosition());

    //// page number
    // retrieve current page number
    if (updatePage) {
      if (id) {
        let page = this.vrvToolkit.getPageWithElement(id);
        // update current score page
        if (page !== null) {
